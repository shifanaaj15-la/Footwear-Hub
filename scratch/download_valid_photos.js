const fs = require('fs');
const path = require('path');
const https = require('https');

const ids = [
  "photo-1549298916-b41d501d3772",
  "photo-1552346154-21d32810aba3",
  "photo-1542291026-7eec264c27ff",
  "photo-1606107557195-0e29a4b5b4aa",
  "photo-1515621061946-eff1c2a352bd",
  "photo-1503642551022-c011aafb3c88",
  "photo-1582588678413-dbf45f4823e9",
  "photo-1516478177764-9fe5bd7e9717",
  "photo-1595341888016-a392ef81b7de",
  "photo-1535043934128-cf0b28d52f95",
  "photo-1595950653106-6c9ebd614d3a",
  "photo-1562273138-f46be4ebdf33",
  "photo-1543163521-1bf539c55dd2",
  "photo-1597045566677-8cf032ed6634",
  "photo-1551107696-a4b0c5a0d9a2",
  "photo-1605348532760-6753d2c43329",
  "photo-1603808033192-082d6919d3e1",
  "photo-1560343090-f0409e92791a",
  "photo-1614252235316-8c857d38b5f4",
  "photo-1575537302964-96cd47c06b1b",
  "photo-1519457431-44ccd64a579b",
  "photo-1608256246200-53e635b5b65f",
  "photo-1518104593124-ac2e82a5eb9d",
  "photo-1514989940723-e8e51635b782",
  "photo-1582966772680-860e372bb558",
  "photo-1509281373149-e957c6296406",
  "photo-1531310197839-ccf54634509e",
  "photo-1637003833874-971d4da7eea6",
  "photo-1509631179647-0177331693ae",
  "photo-1522056683100-34f2f60f0094",
  "photo-1551489186-ccb95a1ea6a3",
  "photo-1718312267162-df826f8ce45f",
  "photo-1524553879936-2ff074ae5816",
  "photo-1457972899686-77aec5e247ce",
  "photo-1515347619252-60a4bf4fff4f",
  "photo-1573100925118-870b8efc799d",
  "photo-1559669615-c03ba52afc37",
  "photo-1562687848-c1664eff566d",
  "photo-1607522370275-f14206abe5d3",
  "photo-1604902396830-aca29e19b067",
  "photo-1539185441755-769473a23570",
  "photo-1608231387042-66d1773070a5",
  "photo-1491553895911-0055eca6402d",
  "photo-1511556532299-8f662fc26c06",
  "photo-1600185365483-26d7a4cc7519",
  "photo-1556909114-f6e7ad7d3136",
  "photo-1600269452121-4f2416e55c28",
  "photo-1533867617858-e7b97e060509",
  "photo-1543508282-6319a3e2621f",
  "photo-1699205017793-f69fb69502d2",
  "photo-1499996860823-5214fcc65f8f",
  "photo-1491897554428-130a60dd4757",
  "photo-1508170754725-6e9a5cfbcabf",
  "photo-1493106819501-66d381c466f1",
  "photo-1515955656352-a1fa3ffcd111",
  "photo-1620045315169-771225b6b45c"
];

const outputDir = path.join(__dirname, 'valid_imgs');
if (!fs.existsSync(outputDir)) {
  fs.mkdirSync(outputDir);
}

function downloadImage(url, dest, callback) {
  const file = fs.createWriteStream(dest);
  https.get(url, (response) => {
    if (response.statusCode === 302 || response.statusCode === 301) {
      downloadImage(response.headers.location, dest, callback);
      return;
    }
    if (response.statusCode !== 200) {
      console.log(`Failed to download ${url}: status ${response.statusCode}`);
      fs.unlink(dest, () => {});
      callback();
      return;
    }
    response.pipe(file);
    file.on('finish', () => {
      file.close(callback);
    });
  }).on('error', (err) => {
    fs.unlink(dest, () => {});
    console.error(`Error downloading ${url}:`, err.message);
    callback();
  });
}

const tasks = ids.map(id => ({
  url: `https://images.unsplash.com/${id}?w=300&auto=format&fit=crop`,
  dest: path.join(outputDir, `${id}.jpg`),
  id
}));

let completed = 0;
function runNext() {
  if (completed >= tasks.length) {
    console.log('All downloads completed!');
    return;
  }
  const task = tasks[completed];
  downloadImage(task.url, task.dest, () => {
    completed++;
    if (completed % 10 === 0) {
      console.log(`Downloaded ${completed}/${tasks.length}...`);
    }
    runNext();
  });
}

runNext();
